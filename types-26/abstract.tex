...
\ensuremath{\Conid{M}^{2}\;\mathbin{:}\;\Conid{Cont}^{2}\;\rightarrow\;\Conid{Cont}} by coinduction-induction shows a positivity issue.
...
we can shot that there is no third order fixed-point operators, \ensuremath{\Conid{W}^{3}}.
...
According to the semantics, \ensuremath{\Varid{intW}\;\mathbin{:}\;(\Conid{Set}\;\Rightarrow\;\Conid{Set})\;\Rightarrow\;\Conid{Set}} gives a least fixpoint of any functor, but we know such thing does not
exist.

One approach to fixing this problem, is to add fixpoint operators (\ensuremath{\mu\;\Varid{and}\;\nu}) to our syntax and define
an alternative semantics. The alternative we have in mind interprets higher containers \ensuremath{\Conid{HCont}\;(\Conid{A}\;\Rightarrow\;\Conid{B})} as functors \ensuremath{\Conid{HCont}\;\Conid{A}\;\Rightarrow\;\Conid{HCont}\;\Conid{B}}. Applying the external fixpoint \ensuremath{\Conid{W}^{3}} to \ensuremath{\Conid{C}} now gives rise to external
\ensuremath{\Varid{extW}\;\mathbin{:}\;\Conid{HCont}\;((\Varid{*}\;\Rightarrow\;\Varid{*})\;\Rightarrow\;\Varid{*})}, which its semantics corresponds to exactly the usual fixed-point operator \ensuremath{\Conid{W}\;\mathbin{:}\;\Conid{Cont}\;\Rightarrow\;\Conid{Set}}.
