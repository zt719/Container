...
We introduce a recursive syntax \ensuremath{\Varid{2}\;\Conid{Cont}} by extending the \ensuremath{\Conid{Cont}} with a family
\ensuremath{\Conid{PF}\;\mathbin{:}\;\Conid{S}\;\rightarrow\;\Conid{Set}} of positions of \ensuremath{\Conid{F}} and an inductive continuation \ensuremath{\Conid{RF}\;\mathbin{:}\;(\Varid{s}\;\mathbin{:}\;\Conid{S})\;\rightarrow\;\Conid{PF}\;\Varid{s}\;\rightarrow\;\Varid{2}\;\Conid{Cont}}.
...
We define a second-order least-fixpoint operator \ensuremath{\Varid{2}\;\Conid{W}\;\mathbin{:}\;\Varid{2}\;\Conid{Cont}\;\rightarrow\;\Conid{Cont}} by
...
\ensuremath{\Varid{2}\;\Conid{M}\;\mathbin{:}\;\Varid{2}\;\Conid{Cont}\;\rightarrow\;\Conid{Cont}} by coinduction-induction shows positivity issue.
...
\ensuremath{\Varid{2}\;\Conid{Cont}\;\cong\;\Conid{HCont}\;((\Varid{*}\;\Rightarrow\;\Varid{*})\;\Rightarrow\;\Varid{*}\;\Rightarrow\;\Varid{*})}. 
...
we can shot that there is no third order fixed-point operators, \ensuremath{\Varid{3}\;\Conid{W}}.
...
If an external fixpoint operator \ensuremath{\Varid{3}\;\Conid{W}\;\mathbin{:}\;\Conid{HCont}\;(((\Varid{*}\;\Rightarrow\;\Varid{*})\;\Rightarrow\;\Varid{*})\;\Rightarrow\;(\Varid{*}\;\Rightarrow\;\Varid{*})\;\Rightarrow\;\Varid{*})\;\rightarrow\;\Conid{HCont}\;((\Varid{*}\;\Rightarrow\;\Varid{*})\;\Rightarrow\;\Varid{*})}
...
Accordint to the semantics, \ensuremath{\Varid{intW}} gives a least fixpoint of any functor, but we know such thing does not
exist.

One approach to fixing this problem, is to add fixpoint operators (\ensuremath{\mu\;\Varid{and}\;\nu}) to our syntax and define
an alternative semantics. The alternative we hanve in mind interprets higher containers \ensuremath{\Conid{HCont}\;(\Conid{A}\;\Rightarrow\;\Conid{B})} as strictly
positive functors \ensuremath{\Conid{HCont}\;\Conid{A}\;\Rightarrow\;\Conid{HCont}\;\Conid{B}}. The external fixpoint \ensuremath{\Varid{3}\;\Conid{W}} now gives rise to external
\ensuremath{\Conid{W}}. Note that this is also consistent with our first-order and second-order semantics.
...
greatest-fixpoint). That is we can define a syntax of \ensuremath{\Conid{Tm}}:
...
together with a normalization function \ensuremath{\Varid{nf}\;\mathbin{:}\;\Conid{Tm}\;\Gamma\;\Conid{A}\;\rightarrow\;\Conid{Nf}\;\Gamma\;\Conid{A}}.
Furthermore, we show \ensuremath{(\Conid{Con,}\;\Conid{Tms,}\;\Conid{Ty,}\;\Conid{Tm})} is a model of simply-typed
$\lambda$-calculus (STLC), where \ensuremath{\Conid{Tms}} is a list of \ensuremath{\Conid{Tm}}. Similarly, we also hope
to prove that the normalized model \ensuremath{(\Conid{Con,}\;\Conid{Nfs,}\;\Conid{Ty,}\;\Conid{Nf})} is also a STLC, where
